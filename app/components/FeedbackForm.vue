...
const state = reactive({
    nombre: props.feedback.nombre,
    email: props.feedback.email,
    mensaje: props.feedback.mensaje,
})
...
<template>
    ...
    <div class=" flex flex-col gap-4 bg-bg-surface border-2 border-brand-gold-soft max-w-xl rounded-2xl mx-auto p-12 hover:border-brand-gold-hover ">
        ...
        <UForm class="flex flex-col gap-6" @submit.prevent="send('submit', state)" :state="state">
        <UFormField label="Nombre" name="nombre">
            <UInput class=" w-full" v-model="state.nombre"/>
        </UFormField>
        <UFormField label="Correo" name="email">
            <UInput class=" w-full" v-model="state.email" />
        </UFormField>
        <UFormField label="Deja aquí tu mensaje de retroalimentación" name="mensaje">
            <UTextarea class="h-full w-full" v-model="state.mensaje" />
        </UFormField>

        <UButton type="submit" class="uppercase font-display flex justify-center" >
